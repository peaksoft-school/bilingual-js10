import { ReactComponent as Globus } from '../../assets/icons/globus.svg'

export const stepperText = [
   {
      title: 'Confirm your English proficiency today! ',
      desciption:
         'For nearly 30 years, learners have turned to Rosetta Stone to build the fluency and confidence they need to speak new languages.',
      img: <Globus />,
      titleColor: '#FE9102',
      background: '#212629',
      id: Math.random().toString(),
   },
   {
      title: 'Your Path to English Fluency Starts Here!',
      desciption:
         'For almost three decades, Rosetta Stone has been the trusted choice for learners looking to develop their language skills and speak with confidence in new languages.',
      img: <Globus />,
      titleColor: '#FFFFFF',
      background: '#FE9102',
      id: Math.random().toString(),
   },
   {
      title: 'Master English with Confidence! ',
      desciption:
         'Join the legacy of learners whove relied on Rosetta Stone for almost 30 years to achieve fluency and self-assured communication in new languages.',
      img: <Globus />,
      titleColor: '#FE9102',
      background: '#212629',
      id: Math.random().toString(),
   },
   {
      title: 'Unlock Your English Proficiency Today!',
      desciption:
         'For nearly three decades, Rosetta Stone has been the go-to choice for learners seeking fluency and self-assurance in speaking new languages.',
      img: <Globus />,
      titleColor: '#FFFFFF',
      background: '#FE9102',
      id: Math.random().toString(),
   },
   {
      title: 'Your Path to English Fluency Starts Here! ',
      desciption:
         'For almost three decades, Rosetta Stone has been the trusted choice for learners looking to develop their language skills and speak with confidence in new languages.',
      img: <Globus />,
      titleColor: '#FE9102',
      background: '#212629',
      id: Math.random().toString(),
   },
]
